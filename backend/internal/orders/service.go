...
func (s *Service) CreateOrder(ctx context.Context, input *CreateOrderInput) (*CreateOrderOutput, error) {
	...
	order, err := s.queries.CreateOrder(ctx, int32(input.UserID), fmt.Sprintf("%.2f", input.TotalAmount), input.Status)
	if err != nil {
		s.logger.Errorf("failed to create order: %v", err)
		return nil, fmt.Errorf("failed to create order: %w", err)
	}

	totalAmount, err := strconv.ParseFloat(order.TotalAmount.String(), 64)
	if err != nil {
...
func (s *Service) ListOrdersByUserID(ctx context.Context, input *ListOrdersByUserIDInput) (*ListOrdersOutput, error) {
	...
	for i, order := range orders {
		totalAmount, err := numericToFloat(order.TotalAmount)
		if err != nil {
			return nil, fmt.Errorf("failed to parse order total amount: %w", err)
		}
